...
export class PendingLocalMediaState {
    ...
    setCameraPending(value: boolean, { awaitEnableConfirmation = false } = {}): void {
    ...
    setMicrophonePending(value: boolean, { awaitEnableConfirmation = false } = {}): void {
...
export class MeetingController {
    ...
    constructor({ room, roomOptions }: { room: RoomClient; roomOptions?: RoomOptions }) {
        this.room = room;
    ...
    get hasParticipantsWithVideo(): boolean {
        const localHasVideo = Array.from(this.livekitRoom.localParticipant.videoTrackPublications.values())
            .some((publication) => !publication.isMuted);
        const remoteHasVideo = Array.from(this.livekitRoom.remoteParticipants.values())
            .some((participant) => Array.from(participant.videoTrackPublications.values()).some((publication) => !publication.isMuted));
    ...
    async configure({ breakoutRoom }: { breakoutRoom?: string } = {}): Promise<void> {
            ...
            await Promise.all([
                ...
                microphoneEnabled
                    ? localParticipant.setMicrophoneEnabled(true)
                        .then(() => this.pendingLocalMedia.setMicrophoneUnavailable(false))
                        .catch((error: unknown) => {
...
export function firstEnabledVideoPublication(participant: Participant): TrackPublication | undefined {
    return Array.from(participant.videoTrackPublications.values() as Iterable<TrackPublication>).find(
