...
async def logged_in_context():
    async with async_playwright() as p:
        ...
        # Fill the app form
        await page.get_by_role("textbox", name="Project ID").fill(
            config.MESHAGENT_PROJECT_ID
        )
        await page.get_by_role("textbox", name="API Key Secret").fill(
            config.MESHAGENT_KEY_ID
        )
        await page.get_by_role("textbox", name="Enter your secret key").fill(
            config.MESHAGENT_SECRET
        )
